...
if [ ! -f $deviceInfoJson ]; then
  ...
  { [ $isAndroid == true ] || [[ $isAndroid == false && -n "$serial" ]]; } && getDeviceInfo
  if [ $isAndroid == true ] && [ $su == true ]; then
  ...
  if [ $isAndroid == true ] || [[ $isAndroid == false && -n "$serial" ]]; then
    for i in "${!all_key[@]}"; do
...
AuroraStoreVersionName=$(jq -r '.[0].tag_name' <<< "$glabApiResponseJson")
[ -z $AuroraStoreVersionName ] && AuroraStoreVersionName="4.7.5"
AuroraStoreVersionCode=$(jq -r '.[0].description' <<< "$glabApiResponseJson" | awk -F'[()]' 'NR==1{print $2}')
[ -z $AuroraStoreVersionCode ] && AuroraStoreVersionCode="71"
...
oauth() {
  ...
  if echo "$auth" | grep -q "error code: 500"; then
    ...
    if [ $isAndroid == true ]; then
      ...
      xdg-open "https://www.cloudflare.com/5xx-error-landing/"
      xdg-open "https://auroraoss.com/"
    fi
    exit 1
  elif [ -n "$(jq -r '.authtoken' <<< "$auth")" ]; then
    # echo "$auth" | jq .  # keep for debugging
    authToken=$(jq -r '.authToken' <<< "$auth")
    if [ $isAndroid == false ] && [ -n "$serial" ] && [ $su == true ]; then
    ...
    elif [ $isAndroid == true ] && [ $su == true ]; then
